...
import { updatePuzzle } from "../graphql/mutations";
export default function PuzzleUpdateForm(props) {
  const {
    id: idProp,
    puzzle: puzzleModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = puzzleRecord
      ? { ...initialValues, ...puzzleRecord }
      : initialValues;
    setDate(cleanValues.date);
    setImageUrl(cleanValues.imageUrl);
    setActualLat(cleanValues.actualLat);
    setActualLng(cleanValues.actualLng);
    setHintRadius(cleanValues.hintRadius);
    setDifficulty(cleanValues.difficulty);
    setImageDescription(cleanValues.imageDescription);
    setAiFunFacts(cleanValues.aiFunFacts);
    setErrors({});
  };
  const [puzzleRecord, setPuzzleRecord] = React.useState(puzzleModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getPuzzle.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPuzzle
        : puzzleModelProp;
      setPuzzleRecord(record);
    };
    queryData();
  }, [idProp, puzzleModelProp]);
  React.useEffect(resetStateValues, [puzzleRecord]);
  const validations = {
    date: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updatePuzzle.replaceAll("__typename", ""),
            variables: {
              input: {
                id: puzzleRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "PuzzleUpdateForm")}
      {...rest}
    >
      <TextField
        label="Date"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || puzzleModelProp)}
